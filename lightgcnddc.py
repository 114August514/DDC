...
DEFAULT_MODEL_FILE = [
    ...
    "./saved/LightGCN-Sep-22-2026_17-41-29.pth",  # tmall (UPDATE THIS)
]
DEFAULT_REP_DIRECTION_FILE = [
    ...
    "./e_pop_saved/tmall/rep_direction_item_tmall_20250802_203218.json",  # tmall (UPDATE THIS, or use the pre-calculated one)
]
...
class LightGCNDebiasFinetune(GeneralRecommender):
    ...
    def __init__(self, config, dataset, pretrained_model, popularity_direction_norm):
        ...
        if self.random_pop_direction:
            self.popularity_direction_norm = F.normalize(
                torch.randn(config["embedding_size"]).to(self.device), p=2, dim=0
            )
        ...
        with torch.no_grad():
            ...
            self.pretrained_model.norm_adj_matrix = (
                self.pretrained_model.norm_adj_matrix.cpu()
            )
            self.user_embeds_orig, self.item_embeds_orig = (
                self.pretrained_model.forward()
            )
            self.user_embeds_orig, self.item_embeds_orig = (
                self.user_embeds_orig.to(self.device),
                self.item_embeds_orig.to(self.device),
            )

            if self.use_epre:
                self.logger.info(
                    "Constructing personalized preference direction (e_pre)..."
                )
                self.e_pre_all_users = self._construct_e_pre(dataset, config).to(
                    self.device
                )
        ...
        self.loss_combination = config[
            "loss_combination"
        ]  # e.g., 'ab_ab', 'a_none', 'b_ab'
    ...
    def _construct_e_pre(self, dataset, config):
        ...
        for user_id in tqdm(range(self.n_users), desc="Constructing e_pre"):
            ...
            if len(interacted_item_indices) == 0:
                ...
                all_e_pre.append(
                    torch.zeros(config["embedding_size"], device=self.device)
                )
                continue

            interacted_items = (
                torch.from_numpy(interacted_item_indices).long().to(self.device)
            )
    ...
    def calculate_loss(self, interaction):
        ...
        for_pos_str, for_neg_str = (
            (parts[0], parts[1]) if len(parts) == 2 else ("ab", "ab")
        )
        if "none" in for_pos_str:
            for_pos_str = ""
        if "none" in for_neg_str:
            for_neg_str = ""

        u_embeds_pos, u_embeds_neg = self._get_modified_user_embeddings(
            user, for_pos_str, for_neg_str
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run Advanced Debiased LightGCN Finetuning"
    )
    ...
    parser.add_argument(
        "--use_epre",
        action="store_true",
        help="Enable learning with personalized preference direction (e_pre).",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="pop",
        choices=["pop", "y_uio", "y_uiprefer", "y_uipop", "pop_ratio", "cosine"],
        help="Sorting mode for e_pre construction.",
    )
    parser.add_argument(
        "--epre_select_mode",
        type=str,
        default="top",
        choices=["top", "bottom"],
        help="Selection mode for e_pre construction.",
    )
    parser.add_argument(
        "--epre_agg_mode",
        type=str,
        default="mean",
        choices=["mean", "orig_weight", "sort_weight", "sort_inv_weight"],
        help="Aggregation mode for e_pre construction.",
    )
    parser.add_argument(
        "--epre_topk",
        type=float,
        default=0.2,
        help="Proportion of items to select for e_pre (e.g., 0.2 for top 20%%).",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0001,
        help="L2 regularization weight for alphas.",
    )

    parser.add_argument(
        "--stopping_step", type=int, default=10, help="Early stopping patience."
    )
    ...
    config["prediction_mode"] = (
        args.pred_mode
    )  # Set prediction mode from command line arguments
    ...
    logger.info(
        f"Popularity direction vector loaded. Shape: {popularity_direction_norm.shape}"
    )
    ...
    if args.use_epre:
        expected_params += finetune_model.n_users
    logger.info(
        f"Number of trainable parameters: {trainable_params} (Expected: {expected_params})"
    )
    ...
    alpha_save_path = os.path.join(
        config["checkpoint_dir"], f"learned_alphas_{TIMESTAMP}.npy"
    )
    ...
    if args.use_epre:
        ...
        beta_save_path = os.path.join(
            config["checkpoint_dir"], f"learned_betas_{TIMESTAMP}.npy"
        )
